...
describe('CalendarIntegration', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Dialog  
    vi.spyOn(globalThis, 'Dialog' as any).mockImplementation((config: any) => ({
      render: vi.fn()
    }) as any);
  });
  ...
  describe('initialization', () => {
    ...
    it('should initialize when Seasons & Stars is available', async () => {
      ...
      expect(Hooks.on).toHaveBeenCalledWith('seasons-stars:calendarGridRendered', expect.any(Function));
      expect(Hooks.on).toHaveBeenCalledWith('seasons-stars:calendarWidgetRendered', expect.any(Function));
