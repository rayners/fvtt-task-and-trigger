...
describe('TaskManager', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(storage, 'removeTask').mockImplementation(async (taskId: string, scope: 'world' | 'client') => {
  ...
  describe('scheduleTask', () => {
    ...
    it('should validate time specification', async () => {
      ...
      await expect(
        taskManager.scheduleTask(invalidTimeSpec, callback, false)
      ).rejects.toThrow();
    });
    ...
    it('should validate callback', async () => {
      ...
      await expect(
        taskManager.scheduleTask(timeSpec, emptyCallback, false)
      ).rejects.toThrow();
    });
  ...
  describe('task management', () => {
    ...
    it('should get all tasks', async () => {
      const taskId1 = await taskManager.scheduleTask({ minutes: 5 }, 'console.log("task1");', false);
      const taskId2 = await taskManager.scheduleTask({ minutes: 10 }, 'console.log("task2");', false);
  ...
  describe('error handling', () => {
    ...
    it('should handle invalid task updates', async () => {
      ...
      await expect(
        taskManager.updateTask(invalidTask as any)
      ).rejects.toThrow();
    });
  });
